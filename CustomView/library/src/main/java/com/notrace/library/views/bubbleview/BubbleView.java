...
public class BubbleView extends RelativeLayout implements Runnable{
    ...
    private void initialize(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.BubbleTextView);
        float radius = a.getDimension(R.styleable.BubbleTextView_bubbleCornerRadius, 0);
        backgroundColor = a.getColor(R.styleable.BubbleTextView_bubbleBackgroundColor, 0);

        pressBackgroundColor = a.getColor(R.styleable.BubbleTextView_bubbleBackgroundPressColor, 0);
        int textColor = a.getColor(R.styleable.BubbleTextView_bubbleTextColor, 0);
        float textSize = a.getDimension(R.styleable.BubbleTextView_bubbleTextSize, 0);
        String contentText = a.getString(R.styleable.BubbleTextView_bubbleText);

        int intDirection = a.getInt(R.styleable.BubbleTextView_bubbleArrowDirection, 3);
        setCurDirection(intDirection);

        float relativePosition = a.getFraction(R.styleable.BubbleTextView_relativePosition, 1, 1, 0.3f);
    ...
    private void initContent(float radius, final int backgroundColor, int textColor, float textSize, String content) {
        ...
        RoundRectDrawable roundRectDrawable = new RoundRectDrawable(backgroundColor, radius);
        ...
        setBackground(new StateColorDrawable(Color.TRANSPARENT) {
            @Override
            protected void onIsPressed(boolean isPressed) {
                RoundRectDrawable conRlBackground = (RoundRectDrawable) conRl.getBackground();
                if (isPressed) {
